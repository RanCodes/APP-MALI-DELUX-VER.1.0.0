...
const WeightManager: React.FC<WeightManagerProps> = ({ weights, onWeightsChange, rates, onRatesChange }) => {
  ...
  const handleBulkUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const skuCol = findColumnByKeywords(columns, ['sku', 'referencia', 'item', 'codigo', 'código', 'part number', 'pn']);
      const weightCol = findColumnByKeywords(columns, ['peso', 'weight', 'kg', 'kilogramos', 'masa', 'kilograms']);
      const nameCol = findColumnByKeywords(columns, ['nombre', 'name', 'descripcion', 'descripción', 'titulo', 'título', 'producto']);

      if (!skuCol || !weightCol) {
        alert(`Faltan columnas: Necesitamos algo parecido a SKU y PESO.`);
        return;
      }
      ...
      data.forEach(row => {
        ...
        const rowName = nameCol ? String(row[nameCol] || '').trim() : undefined;
      ...
      <div className="space-y-8">
        ...
        <div className="bg-white dark:bg-slate-900 p-8 rounded-[2rem] border border-slate-200 dark:border-slate-800 shadow-xl">
          ...
          <div className="space-y-3 max-h-60 overflow-y-auto pr-2 custom-scrollbar">
            ...
            {rates.length === 0 && <p className="text-xs font-bold text-slate-400 text-center py-10 uppercase tracking-widest">Sin escalas definidas</p>}
          </div>
        </div>
        ...
        <div className="bg-indigo-600 dark:bg-indigo-900/40 p-8 rounded-[2rem] border border-indigo-400/20 shadow-2xl relative overflow-hidden group">
          ...
          <h3 className="text-xl font-black text-white mb-2 flex items-center gap-3">Carga Masiva</h3>
          <p className="text-sm font-medium text-indigo-100/80 mb-6">Importa tu base de SKU y pesos desde un archivo Excel o CSV.</p>
          ...
          >
            {isProcessing ? (
              <div className="w-5 h-5 border-4 border-indigo-600 dark:border-white border-t-transparent rounded-full animate-spin"></div>
            ) : (
              <>Subir Archivo de Pesos</>
            )}
          </button>
        </div>
      </div>
        ...
        <div className={`bg-white dark:bg-slate-900 p-8 rounded-[2.5rem] border shadow-2xl flex flex-col h-[700px] transition-all duration-500 ${editingSku ? 'border-amber-500 ring-4 ring-amber-500/10' : 'border-slate-200 dark:border-slate-800'}`}>
          <div className="flex justify-between items-center mb-6">
            <h3 className="text-xl font-black text-slate-900 dark:text-white flex items-center gap-3">
              ...
              {editingSku ? 'Editando Registro' : '2. Inventario de Pesos'}
            </h3>
            {editingSku && (
          ...
          <div className="flex-1 overflow-auto space-y-3 pr-2 custom-scrollbar">
            {filteredWeights.map((w, i) => (
              ...
              >
                <div className="flex-1 min-w-0">
                  ...
                  {w.name && <p className="text-[11px] font-bold text-slate-400 dark:text-slate-500 truncate">{w.name}</p>}
                </div>
                <div className="flex items-center gap-4 ml-4">
                  ...
                  <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                      <button 
                        ...
                        className="p-2 text-blue-600 hover:bg-blue-100 dark:hover:bg-blue-900/40 rounded-xl transition-all"
                        title="Editar"
                      >
                        <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z" /></svg>
                      </button>
                      <button 
                        onClick={() => removeWeight(w.sku)} 
                        className="p-2 text-rose-500 hover:bg-rose-100 dark:hover:bg-rose-900/40 rounded-xl transition-all"
                        title="Eliminar"
                      >
            ...
            {filteredWeights.length === 0 && (
              <div className="text-center py-20">
                <p className="text-slate-300 dark:text-slate-700 font-black text-xl uppercase tracking-widest">Sin resultados</p>
              </div>
            )}
          </div>
